...
def get_density_map(annot_name, mask_coord):
    ...
    img = np.zeros((299,299), np.float32)
    ...
    def add_to_image(image, bbox, coord):
        ...
        xmax = int((xmax - coord[1]) * 299 / float(coord[0] - coord[1]))
        xmin = int((xmin - coord[1]) * 299 / float(coord[0] - coord[1]))
        ymax = int((ymax - coord[3]) * 299 / float(coord[2] - coord[3]))
        ymin = int((ymin - coord[3]) * 299 / float(coord[2] - coord[3]))
...
def crop_image(image_name, mask_pts, save_data = False):
    ...
    x_max = np.max(mask_pts[:,0])
    x_min = np.min(mask_pts[:,0])
    y_max = np.max(mask_pts[:,1])
    y_min = np.min(mask_pts[:,1])
    mask_coord = np.array([x_max, x_min, y_max, y_min])
    croped_image = image[y_min:y_max, x_min:x_max]
    resize_image = cv2.resize(croped_image, (299, 299))
...
if __name__ == "__main__":
    mask_dir = "/home/mscvadmin/traffic_video_analysis/data/Cam253/"
    ...
    #test_mask = "/home/mscvadmin/traffic_video_analysis/data/Cam253/[Cam253]-2016_4_21_15h_150f.msk"
    #image_dir_name = test_mask.replace(".msk", "")
    #image_list = file_io.get_listfile(image_dir_name, "jpg")
    #mask_pts = load_mask(test_mask)
    #for img in image_list:
    #        crop_image(img, mask_pts, True)
